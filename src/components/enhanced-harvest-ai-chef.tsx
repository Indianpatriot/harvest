...
import { getEnhancedRecipeSuggestions, EnhancedRecipe } from '@/ai/flows/enhanced-recipe-suggestions';
import { findRecipesByName, FindRecipesByNameOutput } from '@/ai/flows/find-recipes-by-name';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { useToast } from '@/hooks/use-toast';
import { Upload, Trash2, PlusCircle, ChefHat, Heart, Loader2, Check, X, Pencil, Sparkles, Utensils, Soup, Search, Apple, Carrot, Drumstick, Plus, Database, Settings } from 'lucide-react';
...
export function EnhancedHarvestAiChef({
  ...
  const [suggestedRecipesList, setSuggestedRecipesList] = useState<EnhancedRecipe[]>([]);
  const [savedRecipes, setSavedRecipes] = useLocalStorage<EnhancedRecipe[]>('enhanced-saved-recipes', []);
  const [useOpenFoodFacts, setUseOpenFoodFacts] = useLocalStorage<boolean>('use-open-food-facts', true);
  ...
  const handleGetRecipesByIngredients = () => {
    ...
    startSuggesting(async () => {
      try {
        const result = await getEnhancedRecipeSuggestions({ 
        ...
        toast({
          ...
          description: `Found ${result.length} recipes ${useOpenFoodFacts ? 'using Open Food Facts data' : 'using AI'}.`,
        });
      } catch (error) {
    ...
    if (recipeQuery.trim().length === 0) {
      ...
      startFinding(async () => {
        try {
          ...
          // Convert regular recipes to enhanced recipes format
          const enhancedResult: EnhancedRecipe[] = result.map(recipe => ({
            ...recipe,
            difficulty: 'Medium' as const,
            cuisine: 'International',
            nutritionalHighlights: ['AI Generated Recipe'],
            sourceInfo: {
          ...
          setSuggestedRecipesList(enhancedResult);
  ...
  const toggleFavorite = useCallback((recipe: EnhancedRecipe) => {
  ...
  const isFavorite = useCallback((recipe: EnhancedRecipe) => savedRecipes.some(r => r.name === recipe.name), [savedRecipes]);
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <Label htmlFor="open-food-facts">Use Open Food Facts Data</Label>
                        </div>
                        <Switch
                          id="open-food-facts"
                          checked={useOpenFoodFacts}
                          onCheckedChange={setUseOpenFoodFacts}
                        />
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) => setDietaryRestrictions(e.target.value.split(',').map(s => s.trim()).filter(Boolean))}
                            className="bg-background"
                          />
                        </div>
                      </div>
                      ...
                      {useOpenFoodFacts && (
                        <div className="text-xs text-muted-foreground flex items-center gap-2">
                          ...
                          Nutritional data will be sourced from the Open Food Facts database for more accurate analysis.
                        </div>
                      )}
                    </div>
                  </div>
                   ...
                   <div className="mt-6 flex justify-end">
                     <Button onClick={handleGetRecipesByIngredients} size="lg" className="bg-accent text-accent-foreground hover:bg-accent/90 text-lg py-6 w-full sm:w-auto shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-0.5" disabled={isLoading || ingredients.length === 0}>
                         {isSuggesting ? (
                             ...
                             <>
                               {useOpenFoodFacts && <Database className="mr-2 h-5 w-5" />}
                 ...
                 <div>
                   <div className="flex justify-center mb-6">
                      <div className="bg-card p-1 rounded-full border shadow-sm">
                          <Button variant={activeRecipeTab === 'suggestions' ? 'secondary' : 'ghost'} onClick={() => setActiveRecipeTab('suggestions')} className="rounded-full" disabled={suggestedRecipesList.length === 0}>
                            Suggestions
                            {useOpenFoodFacts && suggestedRecipesList.length > 0 && (
